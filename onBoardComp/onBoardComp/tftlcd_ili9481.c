	...
	for (int j = 0; j < ((fontSizeX_px / 8) * fontSizeY_px); j++) {
		...
		switch (typeFont) {
			...
			case 5:
				...
				break;
			case 6:
				ch = pgm_read_byte(&fontSmall[temp]);
				break;
		}
...
void TFTLCD_ILI9481_graph_print_str_format(int16_t x, int16_t y, char *str, uint16_t color, uint16_t bg, uint8_t typeFont, uint16_t printLen, uint8_t leftOrRightPrint)
{
	int len = strlen(str);

	if (leftOrRightPrint == 0) {
		for (int i = 0; i < len; i++) {
			TFTLCD_ILI9481_graph_print_char(x + (i * fontSizeX_px), y, *str++, color, bg, typeFont);
		}
		
		if (len != printLen && printLen != 0) {
		    uint16_t step = abs(printLen - len);
...
void TFTLCD_ILI9481_graph_print_int_format(int32_t num, int16_t x, int16_t y, uint16_t color, uint16_t bg, uint8_t typeFont, uint16_t printLen, uint8_t leftOrRightPrint)
{
	int length = 0;
	char filler = ' ',
	buf[25],
	...
	TFTLCD_ILI9481_graph_print_str_format(x, y, st, color, bg, typeFont, printLen, leftOrRightPrint);
}

void TFTLCD_ILI9481_graph_print_float(double num, uint8_t dec, int16_t x, int16_t y, uint16_t color, uint16_t bg, uint8_t typeFont, uint16_t printLen, uint8_t leftOrRightPrint)
{
	int length = 0;
	char filler = ' ', divider = '.';
...
void TFTLCD_ILI9481_setFont(uint8_t typeFont)
{
	switch (typeFont) {
		...
		case 5:
			...
			break;
		case 6:
			fontSizeX_px = fontSmall[0];
			fontSizeY_px = fontSmall[1];
			fontOffset = fontSmall[2];
			fontCharnums = fontSmall[3];
		break;
	}
}
